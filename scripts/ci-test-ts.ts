...
function applyChunkBudget(commands: TestCommand[], poolWidth: number): TestCommand[] {
	...
	return commands.map(testCommand => {
		...
		// parallel=0 means plain serial `bun test` with no worker isolation at all:
		// bun 1.4 canaries segfault in `--parallel` worker mode at ANY width, even 1,
		// so the flag must be omitted rather than budgeted down to 1.
		if (testCommand.parallel === 0) {
			return { ...testCommand, command: [...testCommand.command, `--timeout=${timeout}`] };
		}
