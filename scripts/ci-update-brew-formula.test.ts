...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["aura-*"].first` returns `nil`, and
	...
	// popened binary doesn't touch the real `~/.aura` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"aura", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
