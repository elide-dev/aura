...
describe("document conversion cache directory", () => {
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/<app> exists", async () => {
...
describe("legacy file adoption on XDG paths", () => {
	...
	it("adopts legacy files at the XDG paths without clobbering existing XDG files", async () => {
		...
		// Pre-XDG layout: key under ~/<config dir>/agent, registry under ~/<config dir>.
		await fs.mkdir(path.join(tempRoot, getConfigDirName(), "agent"), { recursive: true });
		await fs.writeFile(path.join(tempRoot, getConfigDirName(), "agent", "secret-placeholder.key"), "legacy-key");
		await fs.writeFile(path.join(tempRoot, getConfigDirName(), "marketplaces.json"), '{"legacy":true}');
	...
	it("keeps the legacy paths canonical when XDG is inactive", async () => {
		...
		expect(getSecretPlaceholderKeyPath()).toBe(
			path.join(tempRoot, getConfigDirName(), "agent", "secret-placeholder.key"),
		);
		expect(getMarketplacesRegistryPath()).toBe(path.join(tempRoot, getConfigDirName(), "marketplaces.json"));
	});
});
