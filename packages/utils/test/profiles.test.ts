...
describe("profile env + name validation", () => {
	it("honors AURA_PROFILE > OMP_PROFILE > PI_PROFILE precedence and treats empty/default as the default profile", () => {
		// AURA_PROFILE is canonical and wins over both legacy fallbacks.
		expect(resolveProfileEnv("work", "other", "third")).toBe("work");
		// OMP_PROFILE wins over PI_PROFILE when AURA_PROFILE is undefined.
		expect(resolveProfileEnv(undefined, "work", "other")).toBe("work");
		// PI_PROFILE is consulted only when AURA_PROFILE and OMP_PROFILE are undefined.
		expect(resolveProfileEnv(undefined, undefined, "work")).toBe("work");
		// An explicitly-empty AURA_PROFILE selects the default profile; it must NOT
		// fall through to the lower-precedence OMP_PROFILE/PI_PROFILE.
		expect(resolveProfileEnv("", "work", "other")).toBeUndefined();
		expect(resolveProfileEnv("   ", "work", "other")).toBeUndefined();
		expect(resolveProfileEnv("default", "work", "other")).toBeUndefined();
		// Same explicit-empty-selects-default semantics one level down for OMP_PROFILE.
