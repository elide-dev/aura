...
describe("system prompt tool inventory", () => {
	...
	it("renders runtime selection as inherent policy only for registered capabilities", async () => {
		...
		const renderWith = async (tools: Map<string, SystemPromptToolMetadata>): Promise<string> => {
			const { systemPrompt } = await buildSystemPrompt({
				cwd: tempDir,
				contextFiles: [],
				skills: [],
	...
	it("omits the read-only scout delegation gate when scout is unavailable", async () => {
		...
		expect(withScout).toContain("concurrent main work or read-only scouting");
		expect(withoutScout).toContain("concurrent main work;");
		expect(withoutScout).not.toContain("scouting");
	});
});
