...
test("explicit-only CLI roots replace stale state and exclude every ambient package source", async () => {
	...
	const installed = path.join(home, CONFIG_DIR_NAME, "plugins", "node_modules", "installed-extension");
	...
	writeFile(path.join(project, CONFIG_DIR_NAME, "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	writeFile(path.join(home, ".omp", "agent", "settings.json"), JSON.stringify({ extensions: [userExt] }));
	writeFile(
		path.join(home, CONFIG_DIR_NAME, "plugins", "package.json"),
