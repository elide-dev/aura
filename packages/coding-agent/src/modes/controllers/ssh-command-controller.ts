 ...
 */
import { CONFIG_DIR_NAME, getProjectDir, getSSHConfigPath } from "@oh-my-pi/pi-utils";
	...
	async #handleList(): Promise<void> {
		try {
			...
			if (userHosts.length > 0) {
				lines.push(theme.fg("accent", "User level") + theme.fg("muted", ` (~/${CONFIG_DIR_NAME}/agent/ssh.json):`));
