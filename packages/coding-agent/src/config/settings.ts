	...
	async #saveProjectNow(): Promise<void> {
		...
		try {
			...
			await this.#withYamlWriteLock(projectConfigPath, async writePath => {
				...
				// When the branded file is absent, carry sibling model roles forward without
				// importing anything else a legacy file may hold — see #projectConfigWriteBase.
				const projectSettings =
					loaded ??
					(this.#quarantinedYamlTargets.has(projectConfigPath)
						? structuredClone(this.#projectFileSettings)
						: await this.#projectConfigWriteBase());
