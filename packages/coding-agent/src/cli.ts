...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any AURA_PROFILE/OMP_PROFILE/PI_PROFILE
			// inherited from the environment. Module-load resolution deliberately
			// swallows an invalid value to avoid an uncaught throw before this try/catch
			// is in scope (see `readProfileFromEnvSafe` in dirs.ts), and callers may set
			// AURA_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `AURA_PROFILE=.. omp --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.AURA_PROFILE, process.env.OMP_PROFILE, process.env.PI_PROFILE));
		}
