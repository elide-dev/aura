...
{{#if py}}Top-level `await` works; `asyncio.run(…)` raises error.{{/if}}{{#if pyElide}} A Python cell calling a prelude helper (`read`, `write`, `agent`, …) still works, but may run on a separate CPython kernel that does not share variables with your other Python cells — keep helper calls in their own cells. A bare trailing expression is NOT echoed on this engine: `print()` any value you want to see.{{/if}}
