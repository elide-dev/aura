...
separate stop tool. (If this session has no `hub` tool, nothing here can stop the
job — the result says so and what to do instead; report that to the user.)
`cdp` returns a `ws://` URL; `dap` returns a bare `host:port`. If no endpoint appears within the wait window the job is
