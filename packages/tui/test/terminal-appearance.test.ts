...
describe("ProcessTerminal OSC 11 appearance detection", () => {
	beforeEach(() => {
		...
		delete Bun.env.TMUX;
		// The ConPTY cases below opt IN by setting these explicitly, so clear the
		// ambient ones: on a WSL dev host they would otherwise route every
		// non-ConPTY expectation through upstream's flag-1 carve-out.
