...
// `Uint8Array<ArrayBuffer>`, not the default `Uint8Array<ArrayBufferLike>`: under a DOM
// `lib` the latter is not assignable to `BodyInit`, so the signed re-`fetch` below would
// not typecheck for any consumer compiling this file with DOM types.
async function requestBody(input: string | URL | Request, init?: RequestInit): Promise<Uint8Array<ArrayBuffer>> {
	if (init?.body !== undefined && init.body !== null) {
		if (typeof init.body === "string") return new TextEncoder().encode(init.body);
		if (init.body instanceof Uint8Array) return new Uint8Array(init.body);
