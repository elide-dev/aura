...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		await openHardenedSqlite({
			...
			onOpen: handle => {
				// Before the leases DDL: that is the first lock-taking statement on this
				// connection, and Bun defaults `busy_timeout` to 0.
				SqliteAuthCredentialStore.#installBusyTimeout(handle);
