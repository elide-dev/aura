...
# Every wrapper that execs an LLVM binary interpolates this as {runtime_env}.
# Substituted values are not themselves .format-processed, so the braces here
# are the literal shell ones (no {{}} escaping).
_LLVM_RUNTIME_ENV = """\
# The linux LLVM release binaries link libxml2 dynamically; @llvm_msvc_tools
# vendors a minimal libxml2.so.2 into lib/ so lld-link starts on hosts that
# ship a different soname (or none). Ours comes first — the host copy, if any,
# stays a fallback rather than the ABI the binaries get.
LD_LIBRARY_PATH="$llvm/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
...
def _msvc_cc_impl(rctx):
    ...
    env = _LLVM_RUNTIME_ENV
    rctx.file(
        "bin/clang-cl",
        _CLANG_CL_WRAPPER.format(llvm = llvm, xwin = xwin, runtime_env = env),
        executable = True,
    )
    rctx.file(
        "bin/lld-link",
        _LLD_LINK_WRAPPER.format(llvm = llvm, xwin = xwin, runtime_env = env),
        executable = True,
    )
    for tool in ("llvm-lib", "llvm-rc", "llvm-mt"):
        rctx.file(
            "bin/" + tool,
            _PASSTHROUGH_WRAPPER.format(llvm = llvm, tool = tool, runtime_env = env),
            executable = True,
        )
    ...
    for name in ("ml64.exe", "ml64"):
        rctx.file(
            "bin/" + name,
            _ML64_WRAPPER.format(llvm = llvm, runtime_env = env),
            executable = True,
        )
