...
How the coding agent assembles its system prompt and what users can control with `SYSTEM.md`, `APPEND_SYSTEM.md`, `PREPEND_SYSTEM.md`, `TITLE_SYSTEM.md`, and the matching CLI flags.

Primary implementation:

- `packages/coding-agent/src/main.ts` (`discoverSystemPromptFile`, `discoverAppendSystemPromptFile`, `discoverPrependSystemPromptFile`, `applyResolvedSystemPromptInputs`)
...
| `--prepend-system-prompt <text-or-file>` | CLI                   | Adds a block **ahead of every other block**, including a custom system prompt. Highest prepend precedence. |
| `PREPEND_SYSTEM.md`                     | Discovered config file | Same effect as the prepend flag; used when the flag is absent.                                           |

`SYSTEM.md`, `APPEND_SYSTEM.md`, and `PREPEND_SYSTEM.md` are searched project-first, then user-level. At each scope the config bases are ordered `.omp`, `.claude`, `.codex`, `.gemini`:
...
Prepend is the mirror of append and composes independently of both. `--prepend-system-prompt` / `PREPEND_SYSTEM.md` becomes a new block at index 0, so it is read before the default (or custom) instruction block either way. With all three set the order is: prepend block, custom-or-default block, append text, project/environment footer.

Use prepend for posture the model must read before the harness instructions rather than after them.

## Plain-text contract

`SYSTEM.md`, `APPEND_SYSTEM.md`, `PREPEND_SYSTEM.md`, `--system-prompt`, `--append-system-prompt`, and `--prepend-system-prompt` are plain text. They are values inserted into bundled Handlebars templates; their contents are not recursively compiled as Handlebars.
...
| Put instructions ahead of the harness instructions                                      | `PREPEND_SYSTEM.md` or `--prepend-system-prompt`                         |
