...
3. Active agent `.env` (normally `~/.aura/agent/.env`) for keys whose current value is empty/unset
4. Active config-root `.env` (normally `~/.aura/.env`) for keys whose current value is empty/unset
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`    | Path to the encrypted local broker snapshot cache                                            | Optional in broker mode                                                                                                   | Defaults to `~/.aura/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file.                                                                                                                               |
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.aura/agent/config.yml`; environment variables are fallbacks.
...
| `AURA_RUNTIME_ADAPTER`       | Per-process runtime adapter override: `process`, `embedded`, or `auto`. Invalid values fail startup explicitly. Intended for isolated launches and benchmark containers; stored `runtime.adapter` remains the persistent configuration.                                                                                                                                                                                                                                                                                                                                              |
| `AURA_RUNTIME_AUTO_DOWNLOAD` | Per-process override for the `runtime.autoDownload` setting: `true`/`1`/`yes`/`on` or `false`/`0`/`no`/`off` (case-insensitive; empty values are ignored, invalid ones throw). The benchmark harness forces it `false` inside its containers so a campaign measures the runtime under test instead of a download.                                                                                                                                                                                                                                                                            |
| `AURA_EVAL_JS_ENGINE`        | Per-process override for the `eval.jsEngine` setting: `bun` or `runtime`. Empty values are ignored; an unrecognized value throws. Mirrors `AURA_RUNTIME_ADAPTER`; stored `eval.jsEngine` remains the persistent configuration.                                                                                                                                                                                                                                                                                                                                                              |
...
| `PI_CONFIG_DIR`                                     | Config root dirname under home (default `.aura`)                                                                            |
