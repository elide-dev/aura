...
- In a session without the `hub` tool (`--tools <name>`, or a subagent with IRC disabled) the job still starts, but the guidance says so and points at the `/jobs` picker instead of naming a tool the model does not have. Availability is read from the registry's own active-tool set, not re-derived from hub's gate.
