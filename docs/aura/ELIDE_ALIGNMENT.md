...
| 4 | `docs/settings.md` (doc) **and** `src/config/settings-schema.ts` (code) | Both currently tell the user that selecting `runtime` does nothing: the `eval.jsEngine` docs row and the schema's `runtime` option description each carry the clause *"(no kernel ships yet; falls back to Bun with a notice)"*. **When a kernel lands, delete that clause from both** — the schema string is what `/settings` renders, so leaving it would keep the UI advertising a fallback that no longer happens. An earlier revision of this row claimed the schema was engine-neutral and needed no change; it is not, and the row costs two edits, one of them code. (The option value was `elide` when this row was written; Task 20 renamed it to `runtime` under the naming rule.) |
...
- **`packages/coding-agent/src/tools/eval.ts`** — final as of the env-error
  ...
  falls back to Bun with the "The runtime JS engine is unavailable" notice
  (`:285-292`).
