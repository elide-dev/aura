...
| `packages/coding-agent/src/tools/{jvm-jar,jvm-deps}.ts` | both call the upstream `enforcePlanModeWrite` guard (`tools/plan-mode-guard.ts`) on their output path before dispatching, exactly as `write`/`edit` do. `jvm_jar create` lands a JAR in the working tree and `jvm_deps` writes a report there whenever `output` is set — both through the runtime service, which has no idea a session is planning, so the enforcement can only live in the tool. Without it plan mode's "You NEVER create, edit… working-tree files" contract (`prompts/system/plan-mode-active.md`) had two fork-added holes. `jvm_jar inspect` and an output-less `jvm_deps` write nothing and are not guarded |
| `packages/coding-agent/src/tools/{runtime-serve,runtime-launch}.ts` | `serve` answers to upstream's `launch.enabled` kill switch as well as the fork's `runtime.enabled`: `RuntimeServeTool.createIf` requires both, and `hubToolAvailable` composes tool presence with the setting the way `bash.ts`'s `hasLaunch` hint does. hub enforces `launch.enabled` per-op (`tools/hub/index.ts`), and serve reaches the same broker through `executeLaunch`, so honoring only `runtime.enabled` made the fork's tool a way around a switch the user threw. `jobHandleLine`'s hub-less branch names both reasons ("no hub tool, or process supervision is disabled") instead of asserting the session has no hub tool, which is false for the disabled-supervision case |
| `packages/coding-agent/src/cli/doctor-cli.ts` (tool-gate table) | `ToolGateSettings` carries `launchEnabled` and `serve` has its own predicate reporting whichever of its two gates is off, runtime-first, matching `createIf`'s order. The table is a transcription of the `createIf` gates, so it must move with them; `test/doctor-tool-gate-drift.test.ts` derives the truth from the real registry and fails when it does not |
