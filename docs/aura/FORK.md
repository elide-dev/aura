...
| `packages/coding-agent/src/cli/help-extra.ts` | `getExtraHelpText` documents `AURA_PROFILE` first with `OMP_PROFILE`/`PI_PROFILE` marked legacy; `agents unpack` examples and the profile-alias hint use `${APP_NAME}`/`${CONFIG_DIR_NAME}`. **Known stale (follow-up):** the "Available Tools" list still carries a `run` row (the fork replaced upstream's equally stale `python` row with it) — the tool is retired, so that line advertises something the model cannot call. Replace it with `eval` rather than deleting the row blind; the surrounding list is upstream's and drifts on its own |
