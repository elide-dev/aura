...
| `packages/coding-agent/CHANGELOG.md` | records the fork's user-visible changes under Unreleased: the opt-in embedded runtime adapter (process stays the default), inherent runtime/Superpowers policy, runtime telemetry, and — as breaking Removed entries — the retirement of the `run`, `check`, and `jvm_run` tools |
| `packages/coding-agent/src/modes/theme/loader.ts` | register built-in `aura` and `aura-light` themes in `BUILTIN_THEMES` (imports + entries), mirroring `dark`/`light`. Upstream split the old `theme.ts` monolith by responsibility, so `BUILTIN_THEMES` moved here; `theme.ts` is now a barrel the fork does not touch |
| `packages/coding-agent/src/utils/title-generator.ts`, `src/modes/interactive-mode.ts`, `src/modes/theme/symbols.ts`, `src/modes/theme/defaults/{dark,light}-poimandres.json`, `test/terminal-title-state.test.ts`, `test/title-generator.test.ts` | replace upstream's compact `π` prompt/title brand with Aura's `☉` (the three `icon.pi` entries in the unicode/nerd/ascii presets live in `symbols.ts` since upstream's theme split); preserve `icon.pi` as the compatible custom-theme key, use `o` only for the explicit ASCII preset, and keep title-state behavior unchanged apart from the mark |
...
| `packages/coding-agent/src/tools/renderers.ts` | one import + one `...runtimeToolRenderers` spread at the head of `toolRenderers`, registering the seven runtime tool renderers (`RUNTIME_RENDERER_SPECS` in `tools/runtime-renderer.ts` is the count's source of truth). The spread is first so a future upstream entry with the same key would win rather than be silently shadowed; all renderer logic lives in the fork-owned `tools/runtime-renderer.ts`, so this row stays a two-line change through any merge |
...
| `packages/coding-agent/src/tools/bash.ts` | selects configured rules through `activeBashInterceptorRules(getBashInterceptorRules(), settings.get("bashInterceptor.enabled"))`; the toggle gates every rule, and direct runtime-binary commands are not intercepted. Separately, the description's `hasLaunch` flag is a real conjunction — `isToolActive("hub", true) && settings.get("launch.enabled") === true` — where upstream passed the setting as `isToolActive`'s FALLBACK. Tool presence and the kill switch are independent (hub exists but refuses every op with `launch.enabled=false`), and as a fallback the setting only reached sessions with no `isToolActive` at all, so every `createTools` session was told services MUST use `hub` regardless of the switch. Pinned by `test/bash-launch-guidance.test.ts`. Upstream file, behavioral change — re-check on merge |
| `packages/coding-agent/src/tools/index.ts` | `ToolSession.getRuntimeService?: () => RuntimeService \| undefined` accessor added beside `getMnemopiSessionState`, plus the root-owned `runtimeServiceScope` propagated into every subagent executor; registers **seven** runtime tools into `BUILTIN_TOOLS` — `insights`, `profile`, hub-supervised `serve`, and the four JVM specialists (`jvm_disassemble`, `jvm_format`, `jvm_jar`, `jvm_deps`) — **all `discoverable`**, all on the `runtime.enabled` gate. The fork registers no essential tool: `run` and `check` were retired (`0e7871de4`), and code execution belongs to upstream `eval` |
| `packages/coding-agent/src/tools/{eval-backends,runtime-insights,runtime-profile}.ts`, `src/prompts/tools/runtime-{insights,profile}.md`, `src/session/{agent-session,eval-runner}.ts`, `src/modes/controllers/{input,command}-controller.ts`, `src/modes/utils/hotkeys-markdown.ts`, `src/modes/components/{welcome.ts,tips.txt}`, `src/modes/interactive-mode.ts`, `test/{input-controller-python-prefix,runtime-insights-profile-tools,agent-session-user-shortcut-hooks}.test.ts`, `test/modes/controllers/command-controller-hotkeys.test.ts`, `test/modes/components/welcome.test.ts`, `test/tools/{eval-fallback,eval-description}.test.ts` | Python capability hierarchy: `python.enabled` is the parent for every Python surface; `python.embedded` gates Python in `insights` and `profile`; local `$`/`$$` input is offered when either child key is on, and only `python.enabled` withdraws it. **The `$`/`$$` action executes in the shared CPython kernel — the same one `eval`'s `py` backend uses (`08c50efd8`)** — so a `$` cell and an `eval` Python cell see the same names; it previously dispatched through the retired `run` tool. `user_python` hooks, session history, exclusion semantics, and cancellation ownership are preserved on that path. Disabled surfaces are removed from schemas/descriptions and rejected before dispatch. `PI_PY` may narrow eval but never bypasses the parent. Tests pin routing, both child gates, and parent precedence. |
| `packages/coding-agent/src/tools/{jvm-jar,jvm-deps}.ts` | both call the upstream `enforcePlanModeWrite` guard (`tools/plan-mode-guard.ts`) on their output path before dispatching, exactly as `write`/`edit` do. `jvm_jar create` lands a JAR in the working tree and `jvm_deps` writes a report there whenever `output` is set — both through the runtime service, which has no idea a session is planning, so the enforcement can only live in the tool. Without it plan mode's "You NEVER create, edit… working-tree files" contract (`prompts/system/plan-mode-active.md`) had two fork-added holes. **The guarded path is `path.resolve(session.cwd, output)`, not the raw field, and that is load-bearing:** the guard exempts the session's `local://` sandbox, but the JVM transport knows no URL scheme (`resolveOutputDest` is a bare `path.resolve`), so a raw `local://app.jar` would claim the exemption and then be written to `<cwd>/local:/app.jar` — inside the tree plan mode protects, and the guard's own refusal message names a `local://` path, so it is the model's natural next call. Both fields are documented cwd-relative, so nothing legitimate is lost. `jvm_jar inspect` and an output-less `jvm_deps` write nothing and are not guarded; the `output` test is truthiness, matching the transport's |
| `packages/coding-agent/src/tools/{runtime-serve,runtime-launch}.ts` | `serve` answers to upstream's `launch.enabled` kill switch as well as the fork's `runtime.enabled`: `RuntimeServeTool.createIf` requires both, and `hubToolAvailable` is the conjunction of tool presence and the setting (the same shape `bash.ts`'s `hasLaunch` hint was CORRECTED to — it previously used the setting as a fallback, which is not the same thing). hub enforces `launch.enabled` per-op (`tools/hub/index.ts`), and serve reaches the same broker through `executeLaunch`, so honoring only `runtime.enabled` made the fork's tool a way around a switch the user threw. `jobHandleLine`'s hub-less branch names both reasons ("no hub tool, or process supervision is disabled") instead of asserting the session has no hub tool, which is false for the disabled-supervision case |
| `packages/coding-agent/src/cli/doctor-cli.ts` (tool-gate table) | `ToolGateSettings` carries `launchEnabled` and `serve` has its own predicate reporting whichever of its two gates is off, runtime-first, matching `createIf`'s order. The table is a transcription of the `createIf` gates, so it must move with them; `test/doctor-tool-gate-drift.test.ts` derives the truth from the real registry and fails when it does not. Note the deliberate idiom split: doctor reads `!== false` (matching its siblings and its "unreadable settings ⇒ schema defaults" contract) while the enforcing gates treat a missing value as OFF. They agree for every value the real settings singleton can return, since an unset key yields the schema default |
...
| `packages/coding-agent/src/tools/builtin-names.ts` | appended **seven** runtime names — `insights`, `profile`, `serve`, `jvm_disassemble`, `jvm_format`, `jvm_jar`, `jvm_deps` — to `BUILTIN_TOOL_NAMES`. It is the authority for the count anywhere in these docs. `run` and `check` were appended here originally and removed again in `0e7871de4`; `jvm_run` never shipped a name (no alias, no shim). Each surviving name needs a `docs/tools/<name>.md` page — and only tools that exist may have one, both directions pinned by `test/internal-urls/docs-tool-coverage.test.ts` |
...
| `packages/coding-agent/src/config/settings.ts` (Python capability helpers) | exports the canonical parent-aware `isPythonEnabled`, `isEmbeddedPythonEnabled`, and `isPythonShellEnabled` predicates so eval, runtime tools, and TUI surfaces cannot drift on child/parent precedence |
...
| `packages/coding-agent/src/cli/args.ts` | `Args.prependSystemPrompt`; `Args.runtime` (the `--runtime <path>` flag). (`getExtraHelpText` is re-exported from here but now lives in `cli/help-extra.ts` — see the next row.) |
| `packages/coding-agent/src/cli/help-extra.ts` | `getExtraHelpText` documents `AURA_PROFILE` first with `OMP_PROFILE`/`PI_PROFILE` marked legacy; `agents unpack` examples and the profile-alias hint use `${APP_NAME}`/`${CONFIG_DIR_NAME}`. The "Available Tools" list's stale `run` row (which the fork had put where upstream's equally stale `python` row was) is now `eval`, and upstream's `notebook` row is gone — this build has no `notebook` tool, notebook editing lives inside `edit`/`read`. The list stays a curated SUBSET of `BUILTIN_TOOL_NAMES`, never a superset; `test/cli-help-tool-list.test.ts` derives that from the registry, so the next upstream drift fails there instead of shipping |
...
| `packages/coding-agent/src/session/agent-storage.ts` | clamps the SQLite `-wal`/`-shm` sidecars alongside the database via the shared `hardenSqliteFileModesSync`, not just the db file. WAL frames hold committed row images (cloud refresh tokens included) and SQLite mints the sidecars from the *database file's* mode, so a first run under the default umask would otherwise leave a world-readable pair behind. Placement after `#initializeSchema` is load-bearing — earlier and there are no sidecars to clamp |
| `packages/coding-agent/src/subprocess/worker-client.ts` | import discipline, no behavior change: subpath imports (`/logger`, `/path`, `/worker-host`) instead of the `@oh-my-pi/pi-utils` barrel, plus a lazy `envModule()` accessor. The module is pulled in by worker clients and daemon entries during startup, and the barrel eagerly pulls `env.ts` (applies profile-scoped `.env` files before profile bootstrap) and `ptree` (loads the `pi_natives` addon) — both forbidden there by upstream's own startup-laziness tests. The `runtime/transport/bun` chain that originally put it on the CLI entry graph is gone with the Bun one-shot arm; the discipline stays because the constraint does |
| `packages/coding-agent/src/config/config-file.ts` | adds `getContentSignature()` (`<byteLength>:<Bun.hash base36>`, `null` when absent) for callers caching work derived from a config file. Deliberately not an mtime: Linux stamps file times from a coarse clock, so two writes in one tick are indistinguishable even at nanosecond precision, and an mtime-equality gate drops the second of two quick edits |
| `scripts/ci-test-ts.ts` | bun-version workarounds: `parallel: 0` means plain serial `bun test` with no worker isolation (bun 1.4 canaries segfault in `--parallel` worker mode at ANY width on `packages/ai`, so the flag must be omitted rather than budgeted to 1), and the bun-crash retry additionally detects a crashed *worker* — parent exit 1 with a crash banner in the output — so nondeterministic runtime faults get the same fresh-process rerun |
| CI / bazel-cache migration (`.github/workflows/ci.yml`, `.github/actions/{bazel-cache,bazel-natives,bun-install,native-inputs}/action.yml`, deleted `.github/workflows/bazel-cache-warm.yml`, `scripts/install-tests/run-ci.sh`) | one consolidated row for the fork's CI rework: native addons come from a content-addressed cache keyed by a new `native-inputs` action instead of `npm view @oh-my-pi/pi-natives-*` tarballs (the fork does not publish those packages), BuildBuddy remote cache replaces the retired darwin disk-cache warm workflow, and the release matrix **drops the `darwin-x64` row** (arm64 only). Binary basenames follow `${APP_NAME}` everywhere, so they must stay equal to update-cli's `getBinaryName()` output |
...
| `packages/ai/src/auth-storage.ts` | `AuthStorage` gains the telemetry-only usage-snapshot observer (`setUsageSnapshotListener`, `#usageEntriesFromReport`, `#notifyUsageSnapshots{,FromReport}`) so subscription utilization exports fire on every fresh snapshot even when the store has no durable history (broker-backed remote stores) and even when a store write throws. Observer failures must never affect auth/usage paths |
| `packages/ai/src/auth/sqlite-credential-store.ts` | `SqliteAuthCredentialStore.open` delegates the `0700` parent dir, `0600` database mode, and bounded `SQLITE_BUSY` open retry to the shared `openHardenedSqlite` from `@oh-my-pi/pi-utils/sqlite-hardening`, and re-exports that module's `isSqliteBusyError` so it stays the single copy of the policy. The store is constructed INSIDE `onOpen` (inside the retried region) because the constructor prepares statements and `sqlite3_prepare_v2` can itself return `SQLITE_BUSY`. Upstream extracted this class out of `auth-storage.ts`, so the hardening lives here now — re-apply it to this module, not to `auth-storage.ts`, on future merges |
...
| `packages/coding-agent/src/vibe/runtime.ts` | carries the originating top-level session's root runtime scope through background vibe worker spawns. The identity pin now lives in the fork-owned `test/vibe/vibe-runtime-scope.test.ts`: upstream deleted its own `test/vibe/vibe-runtime.test.ts` (2216 lines, removed inside a commit titled only `chore(changelog): normalized unreleased entries`) while still maintaining `src/vibe/runtime.ts`, and that file also hung at collection, so the fork's contract was extracted rather than carried on top of a deleted upstream suite |
...
| `packages/metaharness/src/runtime-benchmark.ts` | orchestrates balanced matched agent arms over the seven discoverable runtime tools (`RUNTIME_TOOL_NAMES`, `:30-38`) against a `BASELINE_TOOLS` arm; the `run`/`check` essentials it used to add are retired, so the runtime arm's advantage now has to show up through `eval` adoption rather than through a bespoke tool. Injects repository-packaged process and embedded runtime artifacts into source-mounted task containers only when `--embedded-lib` is supplied (the gate is `opts.runtimeBinary || opts.embeddedLib` and then demands both; `runtimeBinary` is not a CLI flag — it is derived from `--embedded-lib` via `packagedRuntimeBinaryForLibrary()`, i.e. `dirname(lib)/../bin/elide` — so omitting `--embedded-lib` passes no `AURA_RUNTIME_*` env at all and the runtime arm silently measures bash fallback), blocks model launches on the deterministic TypeScript verifier smoke, resumes interrupted frozen campaigns without rerunning completed Harbor jobs, reconstructs full arm elapsed time, aggregates paired effectiveness/latency/token/adoption evidence, and freezes the verifier Bun version/SHA-256 beside per-task tools and hashes |
...
| `docs/settings.md`, `docs/environment-variables.md` | documents `runtime.enabled` / `runtime.adapter` / `runtime.autoDownload` / `runtime.path` / `runtime.version` / `runtime.embeddedPath`, including process-default adapter selection, the validated per-process `AURA_RUNTIME_ADAPTER` override, explicit-embedded no-fallback behavior, embedded-library resolution precedence, off-pin managed-version verification limits, and the **seven-tool** runtime surface (`docs/settings.md` §Runtime; the count tracks `BUILTIN_TOOL_NAMES`); documents the parent `python.enabled`, embedded child `python.embedded`, default-off subprocess-fallback child `python.shell`, and non-bypassing `PI_PY` precedence. Both files also carry the branded config paths (`~/.aura/agent`, `<cwd>/.aura`) plus one explicit note each on what the legacy `.omp` dirs still do |
...
| `packages/coding-agent/src/config/settings-schema.ts` (eval.jsEngine), `src/eval/elide/settings.ts`, `src/eval/elide/index.ts`, `src/tools/eval.ts` | fork-added `eval.jsEngine` enum picks the engine for `language: "js"` cells: `bun` (default) or `runtime`. The value is `runtime`, NOT the product name — it is typed into config files and `AURA_EVAL_JS_ENGINE`, rendered by `/settings`, and echoed in the fallback notice the model reads ("The runtime JS engine is unavailable; ran on the Bun engine."), so the naming rule below applies. `elide` is rejected rather than aliased, and the STORED value is validated the same way the env override is — nothing between the settings document and `resolveJsEvalEngine` checks the enum, so an unrecognized value would otherwise miss the `runtime` branch and run every cell on Bun with no notice, silently ignoring a setting the docs promise is an error. The internal directory and identifiers under `src/eval/elide/` keep the product name (code, not UI), but the kernel/worker ERROR strings say "runtime JS kernel": those reach the model once a kernel lands |
...
| Windows Terminal / WSL / tmux rendering (`packages/tui/src/terminal-capabilities.ts`, `packages/coding-agent/src/modes/{interactive-mode.ts,theme/theme-class.ts}`, `packages/coding-agent/src/config/settings-schema.ts`) | aura hardening for the common WSL setup: `detectTerminalId` treats `WT_SESSION` as truecolor (Windows Terminal never emits `COLORTERM`; WSLENV forwards only `WT_SESSION`), `tui.scrollbackRebuild` defaults on for ConPTY hosts, and a new `TERMINAL.italic` capability (`shouldEnableItalicByDefault` / `italicUserOverride`, `PI_FORCE_ITALIC`/`PI_NO_ITALIC`) suppresses SGR 3 under screen-family terminfo (no `sitm`) so tmux/screen no longer paint italic as reverse-video blocks. `theme.italic` and the HUD-note marker in `interactive-mode.ts` gate on it |
| Release pipeline | `scripts/ci-release-build-binaries.ts` outfiles templated `${APP_NAME}-<os>-<arch>`; `.github/workflows/ci.yml` matrix/codesign paths and `release_brew` (elide-dev/homebrew-tap, `Formula/aura.rb`); `scripts/ci-update-brew-formula.ts` (repo `elide-dev/aura`, class `Aura`, installs `aura`); `scripts/install.sh` + `scripts/install.ps1` (fork repo, `aura-*` assets, installs `aura`); `packages/metaharness/src/launch-args.ts` prebuilt names. Asset basenames must stay equal to update-cli's `getBinaryName()` output |
...
### Upstream tests made host-hermetic for WSL

Aura is developed on WSL, and several upstream suites read ambient env at
assertion time, so they pass on upstream's non-WSL CI and fail here. These
carry a fork-added env pin; the assertions themselves are upstream's:

- `packages/tui/test/{kitty-keyboard-da1-ordering,emergency-restore-altscreen,terminal-appearance}.test.ts`
  clear `WSL_DISTRO_NAME`/`WSL_INTEROP` in `beforeEach` (restoring in
  `afterEach`). Upstream's `isConPTYHosted()` carve-out — added in
  `fix(tui): use Kitty flag 1 on Windows to fix Shift+letter input` — downgrades
  the kitty push from `\x1b[>5u` to `\x1b[>1u` on Windows AND WSL, so on a WSL
  host these flag-5 contracts otherwise assert against the ConPTY path. The
  suites' own ConPTY cases opt IN by setting those vars explicitly, which is
  why clearing the ambient ones is the file's original design intent.
- `packages/tui/test/terminal-capabilities.test.ts` pins
  `getTerminalInfo("warp", "linux", {})` for the Kitty-capable case rather than
  reading the host env; the Warp-on-WSL carve-out keeps its own hermetic cases.

**A brand-new upstream test can also bypass the rebrand without conflicting.**
`title-generator.test.ts`'s WSL case arrived from upstream hardcoding `π` and
...
- `packages/utils/src/sqlite-hardening.ts`, `packages/utils/test/sqlite-hardening.test.ts` —
  the single copy of aura's SQLite-at-rest policy: `openHardenedSqlite` (0700 parent
  dir, 0600 database, bounded `SQLITE_BUSY` open retry), `hardenSqliteFileModesSync`
  for the `-wal`/`-shm` sidecars, and `isSqliteBusyError`. Consumed by
  `packages/ai/src/auth/sqlite-credential-store.ts` and
  `coding-agent/src/session/agent-storage.ts` so neither can drift

- `packages/coding-agent/src/telemetry/` — the whole OpenTelemetry export stack,
  fork-added (upstream has no `src/telemetry/`): `init.ts` (env/settings tiering,
  provider registration, the log bridge, and the built-in destination
  `BUILTIN_TELEMETRY_ENDPOINT` = `https://aura.elide.events` with its atomic
  `x-aura-install-id` header — no credential), `events.ts` (the typed bus every
  publisher writes to, including bounded `runtime.call.completed`), `sink-otlp.ts`
  (sole subscriber; maps events onto instruments and log records with no source,
  arguments, output, paths, or exception messages), `metrics.ts` (every `aura.*`
  instrument, incl. `aura.runtime.calls` / `aura.runtime.duration`), `identity.ts`,
  `cloud-session.ts`, `authorized-exporters.ts`, `publishers/`. `docs/telemetry.md`
  is its user-facing contract and is a privacy document — keep it byte-accurate to
  `init.ts`

- `packages/coding-agent/src/cloud/`, `packages/coding-agent/test/cloud/` —
  fork-added Aura cloud client (also absent upstream): `auth.ts`
  (`AuraAuthClient`), `token-manager.ts` / `token-store.ts`, `deployment.ts`,
  `errors.ts`, and the `index.ts`/`internal.ts` split that keeps the public
  surface pinned by `test/cloud/public-contract.test.ts`. It is what supplies
  the optional authorized telemetry transport in `telemetry/init.ts`
...
- `packages/coding-agent/src/runtime/` — runtime capability core; `index.ts` owns the
  ...
  process/embedded routing and composed status matrix, including optimistic embedded
  Java/Kotlin dispatch with process fallback on unsupported language — every language
  now takes that path, the Bun one-shot arm (`transport/bun.ts` plus its
  `bun-run-entry.ts` CLI worker-host re-entry) having been deleted with the `run` tool
  that was its only caller; `transport/local.ts`
...
- `packages/coding-agent/test/runtime-*.test.ts`, `test/doctor-cli.test.ts`,
  ...
  `packages/utils/test/branding.test.ts` — fork-owned
...
- `docs/tools/{insights,profile,serve,jvm_disassemble,jvm_format,jvm_jar,jvm_deps}.md` — root docs pages required by the `omp://` docs-coverage guard. `test/internal-urls/docs-tool-coverage.test.ts` asserts the coverage in **both** directions: one page per `BUILTIN_TOOL_NAMES` entry, and no page without a tool. The reverse assertion exists because `omp://` serves every file under `docs/`, so a page left behind by a retired tool keeps advertising it to the model — which is exactly what `docs/tools/jvm_run.md` did from `2909c71cf` (2026-07-30, which deleted `src/tools/jvm-run.ts`) until 2026-08-11. `run.md`/`check.md` went with the tools in `0e7871de4`; `jvm_run.md` was the orphan the reverse assertion caught
- `docs/telemetry.md` — fork-added, and the only user-facing description of what
  aura exports by default and where. Treat it as a privacy contract: every claim
  in it (destination, headers, credential-or-not, tier order) must match
  `packages/coding-agent/src/telemetry/init.ts` exactly
