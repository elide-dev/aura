...
| `packages/coding-agent/src/tools/{eval-backends,runtime-run,runtime-insights,runtime-profile}.ts`, `src/prompts/tools/runtime-{run,insights,profile}.md`, `src/session/{agent-session,eval-runner}.ts`, `src/modes/controllers/{input,command}-controller.ts`, `src/modes/utils/hotkeys-markdown.ts`, `src/modes/components/{welcome.ts,tips.txt}`, `src/modes/interactive-mode.ts`, `test/{input-controller-python-prefix,runtime-run-tool,runtime-insights-profile-tools,agent-session-user-shortcut-hooks}.test.ts`, `test/modes/controllers/command-controller-hotkeys.test.ts`, `test/modes/components/welcome.test.ts`, `test/tools/{eval-fallback,eval-description}.test.ts` | Python capability hierarchy: `python.enabled` is the parent for every Python surface; `python.embedded` gates Python in `run`, `insights`, and `profile`; local `$`/`$$` input rides embedded Python, with `python.shell` adding a subprocess-interpreter fallback when embedded is off; either route keeps the action and its welcome/hotkey guidance. Enabled shell actions execute through the registered `run` tool while preserving `user_python` hooks, session history, exclusion semantics, and cancellation ownership. Disabled surfaces are removed from schemas/descriptions and rejected before dispatch. `PI_PY` may narrow eval but never bypasses the parent. Tests pin routing, both child gates, and parent precedence. |
...
| `packages/coding-agent/src/cli/args.ts` | `Args.prependSystemPrompt`; `Args.runtime` (the `--runtime <path>` flag); `getExtraHelpText` documents `AURA_PROFILE` first with `OMP_PROFILE` marked legacy and advertises the managed multi-language `run` tool instead of the legacy Python tool/action; `agents unpack` examples use `${APP_NAME}` and `${CONFIG_DIR_NAME}` |
