...
| `packages/coding-agent/src/subprocess/worker-client.ts` | import discipline, no behavior change: subpath imports (`/logger`, `/path`, `/worker-host`) instead of the `@oh-my-pi/pi-utils` barrel, plus a lazy `envModule()` accessor. The module sits on the CLI entry graph via `runtime/transport/bun`, and the barrel eagerly pulls `env.ts` (applies profile-scoped `.env` files before profile bootstrap) and `ptree` (loads the `pi_natives` addon) — both forbidden there by upstream's own startup-laziness tests |
