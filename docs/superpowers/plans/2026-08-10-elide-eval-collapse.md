...
# Addendum 2 (2026-08-11): M3 — wire the Tier 2 artifact (READY, awaiting digest-final flag + JIT-ON build)

Trigger: WHIPLASH agent flags "digest final"; new artifact staged in `out/`. Facts from its handoff (authoritative doc arrives with the artifact): OQ9 RESOLVED — envelope = new arms on the existing `EmbeddedResponse` union (inherits protocolVersion/requestId); `outputLimitExceeded` is a FIFTH `EmbeddedEvalResult.outcome` arm so `contextAlive` rides the same struct. Spike A = YES (JVM-verified; native preinit of language-restricted contexts UNVERIFIED — probe first, Task 26). NEW CONTRACT beyond the queue doc: survivable guest exit — `exit(N)` ends only that eval, surfaces isExit/exitStatus, guest state intact (IPython-like; pin it). HARD CONSTRAINT: streaming has no wall-clock deadline; a parked write wakes only on host drain, control op, or interrupt → the pump MUST run on a second thread (our control/execution split, now mandatory). Do NOT chase two known pre-existing WHIPLASH failures (smoke-suite process-poisoning when run whole; intermittent child-cleanup flake).

### Task 24: Repin + regenerate + transport plumbing

Order is load-bearing: (1) ABI pin 1→2 in `scripts/sync-embedded-runtime-protocol.ts` (~:313) FIRST; (2) `bun scripts/sync-embedded-runtime-protocol.ts --whiplash <worktree>` — closure 14→15 files (`guest.ts`), `schema.ts` rewritten (NOTE: repin is ONE-WAY; the ABI-1 artifact stops passing integration tests); (3) `abi.ts` adds `elide_embed_context_call`/`elide_embed_context_control` (same `{args:["u64","ptr","usize","ptr"],returns:"i32"}` family, `consumeResponse` routing, caller-frees unchanged); (4) `codec.ts` encode/decode for `EmbeddedContextCall`/`EmbeddedControl` roots, the new response arms per OQ9, new failure codes, the fifth outcome arm; (5) `worker-protocol.ts` context ops (context-call → execution worker; open/close/interrupt/cancel/reset/poll-output → control worker); (6) `worker-core.ts` routing + the output pump on the CONTROL worker thread (hard requirement above; reuse the cancelEmbeddedRequestUntilSettled cross-thread pattern). Tests: extend runtime-embedded-{abi,codec,worker}; re-point integration suites at the ABI-2 artifact and document the new count. Gate: hermetic suites + artifact suite vs NEW artifact + check:ts.

### Task 25: Elide JS/TS kernel factory + parity swap

The ≤4-file wiring from ELIDE_ALIGNMENT's checklist: `eval/elide/kernel-embedded.ts` implementing `ElideJsKernelFactory` over the context ops (contextOpen spec: languages [js,ts], streamOutput per pump; send(run)→contextCall; onMessage←pump; interrupt/reset/close→ops); RESOLVE the parked install-site design call (process-wide slot vs per-scope — the checklist's three options; LAND A TEST WITH IT per the recorded obligation); flip the parity suite's factory line + the TODO(m3) strengthenings (revert state carrier to `var`; re-point interrupt at a genuinely in-flight cell); ADD the survivable-exit pin (cell calls exit(3) → eval ends with exitStatus 3, next cell still sees prior globals). eval.md prompt: make the Bun-globals promise engine-aware NOW (the gap recorded in the checklist) since the engine can actually serve cells.

### Task 26: Python engine (Sam's directive: ONE Python runtime = Elide, `$` included)

FIRST ACT: native-artifact thread probe — python-only context via the new ABI, `threading.Thread(...).start()` must succeed on the NATIVE image (spike A was JVM-only). If it fails, STOP and report (the directive's gate). Then: `eval.pyEngine` setting mirroring jsEngine (bun→cpython? value naming: `cpython|elide` — match the jsEngine bun|elide pattern; default cpython); Elide python ExecutorBackend + kernel factory (python-only ContextSpec, allowThreads=true, mainScript mode for file semantics; NO runner.py orphan-guard — in-process, nothing to orphan); `$` inherits via the eval kernel path automatically — add one $-python-on-elide test. Benchmark gate before default-flip: python warm startup must beat the CPython subprocess (16-18ms) — the whole point of Tier 2.

### Task 27: Bench + gate + ship

New adapter cases: persistent-context warm eval vs per-call vs process; python-vs-CPython-subprocess; reset cost; first-chunk latency (streaming); interrupt latency. Floors guarded (JS/TS warm 3.4×/3.0×, cancellation ~1.1ms abs, Java 0.19× basis-adjusted). Record the sweep; full suite; update FORK.md/CHANGELOG/docs (incl. folding the handoff doc's contract additions into WHIPLASH_QUEUE/ELIDE_ALIGNMENT — survivable exit, OQ9 resolution, spike-A caveat outcome); push to the PR.
