...
| `eval.pyEngine` | enum | `elide` | Engine that executes Python eval cells: `elide` (the managed Elide runtime, which serves cells from a persistent guest context) or `cpython` (the kernel subprocess). The `elide` engine falls back to CPython when no runtime library resolves — see `runtime.embeddedPath` / `AURA_RUNTIME_EMBEDDED_LIB`; the fallback is announced with a notice only when you selected `elide` yourself (setting or env var), since the default selection falling back is not something you asked for. The `elide` engine has no eval tool bridge yet, so a cell calling `read()`/`write()`/`agent()` is routed to the CPython engine with a notice. **That cell runs in the CPython kernel's own session state:** it does not see variables set by cells that ran on the `elide` engine, and variables it defines are invisible to later `elide` cells. Keep helper calls in their own cells, or set `eval.pyEngine` to `cpython` for one state universe throughout. `AURA_EVAL_PY_ENGINE` overrides it for the process; an unrecognized value is an error. |
...
| `lsp.shared` | boolean | `true` | Share one language server per project across local `aura` processes through the daemon broker; falls back to private servers when the broker is unavailable. |
