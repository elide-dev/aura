...
Doctor's tool section is derived from the tool registry rather than from a list: every builtin factory is constructed against a stub session carrying the settings above, and the ones that decline are reported as gated off with the setting responsible (`memory.backend = off`, `runtime.enabled = false`, …). Tools whose registration depends on the live session or an external program — `ask`, `checkpoint`, `rewind`, `lsp`, `github` — are listed separately and counted as registered, because doctor deliberately builds no session and runs no external probe to decide them.
