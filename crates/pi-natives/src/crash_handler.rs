...
//!   `$XDG_STATE_HOME/aura/logs/` on Linux / macOS when the user has migrated
...
const DEFAULT_CONFIG_DIR: &str = ".aura";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/aura/`),
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/aura/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/aura` exists on disk, and
...
/// `<xdg_state_home>/aura` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
...
mod tests {
	...
	#[test]
	fn resolve_logs_dir_defaults_under_dot_aura() {
		let dir = resolve_logs_dir(Path::new("/tmp/pi-natives-test-home"), None, None);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.aura/logs"));
	}
	...
	fn resolve_logs_dir_honors_relative_pi_config_dir() {
		let dir = resolve_logs_dir(
			...
			Some(OsStr::new(".aura-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.aura-dev/logs"));
	}
	...
	fn resolve_logs_dir_normalizes_parent_components_like_path_join() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			Some(OsStr::new("nested/../.aura-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.aura-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		let dir = xdg_state_logs(
			...
			Some(OsStr::new("")),
			Path::new("/tmp/pi-natives-test-home/.aura/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/aura/logs")));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/aura/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/aura/logs"));
	}
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/pi-natives-test-home/.aura/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.aura/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.aura/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/aura/logs")));
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_aura_by_default() {
		let dir = default_agent_dir(Path::new("/tmp/pi-natives-test-home"), None);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.aura/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_respects_pi_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/pi-natives-test-home"), Some(OsStr::new(".aura-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.aura-dev/agent"));
	}

	#[test]
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/pi-natives-test-home/.aura/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/pi-natives-test-home/.aura/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/pi-natives-test-home/.aura/logs/native-alloc-99-1.log")
		);
	}
}
